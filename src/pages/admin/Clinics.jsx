...
const EMPTY = { name_en: '', name_ar: '', department_id: '', is_active: 1 };
...
export default function Clinics() {
  ...
  const load = async () => {
    try {
      const [clinicsRes, deptsRes] = await Promise.all([
        axios.get('/api/clinics'),
        axios.get('/api/departments'),
      ]);
      setItems(clinicsRes.data);
  ...
  const openEdit = (item) => {
    setForm({ name_en: item.name_en, name_ar: item.name_ar, department_id: item.department_id || '', is_active: item.is_active });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await axios.put(`/api/clinics/${editing}`, payload);
      } else {
      ...
      setError(err.response?.data?.message || 'Error saving clinic');
    }
  };

  const handleDelete = async () => {
    try {
      await axios.delete(`/api/clinics/${deleteTarget}`);
      setDeleteTarget(null);
      load();
    } catch (err) {
      setDeleteTarget(null);
      alert(err.response?.data?.message || 'Error deleting clinic');
    }
  };
  ...
  return (
    <div className="page">
            ...
            <tbody>
              {items.map((item, i) => (
                <tr key={item.id}>
                  ...
                  <td>{item.name_ar}</td>
                  <td>{item.name_en}</td>
                  <td>{isRTL ? item.dept_name_ar : item.dept_name_en || '—'}</td>
                  <td>
              ...
              {items.length === 0 && (
                <tr><td colSpan={6} className="empty-row">{isRTL ? 'لا توجد عيادات' : 'No clinics found'}</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {showModal && (
        <Modal title={editing ? (isRTL ? 'تعديل عيادة' : 'Edit Clinic') : (isRTL ? 'إضافة عيادة' : 'Add Clinic')} onClose={closeModal}>
      ...
      {deleteTarget && (
        <ConfirmDialog
          message={isRTL ? 'هل أنت متأكد من حذف هذه العيادة؟' : 'Are you sure you want to delete this clinic?'}
